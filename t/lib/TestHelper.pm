...
sub mock_execute_script_and_check_sorted_output {
    my ( $script_name, $scripts_and_expected_files, $columns_to_exclude ) = @_;
    open OLDOUT, '>&STDOUT';
    open OLDERR, '>&STDERR';
    eval("use $script_name ;");
    ...
    my $returned_values = 0;
    {
        ...
        for my $script_parameters ( sort keys %$scripts_and_expected_files ) {
            ...
            for (my $i=0; $i < scalar(@actual_output_file_names); $i++) {
                ...
                if ( defined($expected_output_file_names[$i]) ) {
		    sort_file($actual_output_file_names[$i], $actual_output_file_names[$i] . '.sorted'); 
                    sort_file($expected_output_file_names[$i], $expected_output_file_names[$i] . '.sorted');

                    compare_ok( $actual_output_file_names[$i] . '.sorted', $expected_output_file_names[$i] . '.sorted', "Actual and expected output match for '$script_parameters'" );
                }

		unlink($actual_output_file_names[$i]);
		unlink($actual_output_file_names[$i] . '.sorted');
		unlink($expected_output_file_names[$i] . '.sorted');
            }
        }
        close STDOUT;
        close STDERR;
